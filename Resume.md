...
- **Built the pipeline consuming Snowplow Signals**, one of the product's first pilot customers — feeding back gaps (e.g. coarse event aggregation) that Snowplow shipped fixes for. Used it to build per-user behaviour profiles from site activity (search frequency, hint engagement) driving targeted in-app nudges.
