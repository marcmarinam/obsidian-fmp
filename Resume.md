...
- **Own the CRM and messaging integration (Iterable)** behind in-app messaging and email campaigns — built in-house dismiss-state tracking and Zod-based schema validation to cover gaps in Iterable's own logic, plus a pipeline on Snowplow Signals (an early pilot deployment, feeding back gaps that shipped fixes) to build per-user behaviour profiles driving targeted nudges; my main focus for the last two years.
